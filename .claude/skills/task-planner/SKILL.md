...
8. **阶段集成 Gate**：每个阶段必须定义集成验收清单，确保该阶段所有 Task 联调后功能可用
9. **用中文输出**
...
|--------|---------|-----------|---------|
| 项目可构建 | 运行构建命令 | 0.1, 0.2 | 构建成功，无错误 |
| 数据库可连接 | 运行 health check | 0.2, 1.1 | 返回 200 |

### 阶段 1 集成验收

| 验收项 | 验证方式 | 涉及 Task | 通过标准 |
|--------|---------|-----------|---------|
| 注册→登录全链路 | curl 注册 → curl 登录 | 1.1, 1.2, 1.3 | 注册 201 + 登录 200 |
| JWT 保护路由 | 登录获取 token → 携带 token 访问受保护 API | 1.2, 1.3 | 受保护 API 返回 200，无 token 返回 401 |
...
| 优先级 | P0 |
| 依赖 | [无 / Task A.B] |
| 被依赖 | [无 / Task C.D, Task E.F（哪些 Task 依赖本 Task）] |
| 阶段 | 阶段N: XXX |
| 预估工时 | 2-4 小时 |
| 所属集成 Gate | [阶段 N 集成验收清单] |
...
- 总 Task 数量
- 各阶段 Task 分布
- 每个阶段的集成验收清单概要（验证链路 + 涉及 Task）
- Task 目录结构概要
- 建议首先执行的 Task
- 提示："可使用 ai-master 继续推进项目，自动开始执行第一个 Task"
- 提示："每个阶段完成后 ai-master 会自动执行集成验收 Gate，确保跨 Task 功能可用"
