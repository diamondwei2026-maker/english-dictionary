...
4. **只调度不生产**：你不生成 PRD、不设计架构、不做审查——你只判断该调用哪个子 skill 然后调用它，或在 Coding Prompt 就绪后按指令执行开发
...
1. 读取「下一步」字段的值（如 `Step 3`、`done` 等）和「提交 SHA」字段
2. **Bug 关键词始终优先**：如果用户输入包含 bug 报告特征（"不工作"、"报错"、"bug"、"fix"、"修"、"有问题"、"异常"、"出错"），忽略检查点，直接执行步骤 0
3. **变更驱动复查**：先判断代码是否在两次调用间发生变化：

   a. 执行 `git rev-parse HEAD` 获取当前 HEAD SHA
   b. 与检查点的「提交 SHA」对比：
      - **相同** → 代码无变更，进入步骤 3（跳转前定向验证）
      - **不同** → 执行 `git diff --name-only <旧SHA> <新SHA>` 获取变更文件列表，按变更范围决定复查深度：

      | 变更文件匹配模式 | 复查动作 | 说明 |
      |-----------------|---------|------|
      | 仅匹配 `figma/plans/*.md` | 重查 Step 0.6（figma-sync） | 新 Figma plan 到达 |
      | 仅匹配 `.docs/prd/*.md` 或 `.docs/adr/*.md` | 重查 Step 1 + Step 2 | PRD/ADR 变更需重新规划 |
      | 仅匹配 `.docs/development-plan.md` 或 `.docs/tasks.md` | 重查 Step 1 + Step 2 | 开发计划/任务变更 |
      | 匹配 `client/src/**` 或 `server/src/**` | 跳过 Step 0~2，直接进入 Step 7（代码审查） | 代码变更只需审查 + Gate |
      | 仅匹配非代码文件（`README.md`、`.gitignore` 等） | 不重查，直接按检查点跳转 | 无关文件变更 |
      | 无变更文件（diff 为空） | 不重查，直接按检查点跳转 | 工作区干净 |
      | 多类匹配 | 取优先级最高的受影响步骤，合并重查 | 例如：同时改了 PRD 和代码 → 重查 Step 1 + Step 7 |

      > 变更驱动的复查范围**仅限受影响步骤**。未匹配的步骤保持跳过。复查结果若与检查点不一致，以复查结果为准并更新检查点。

4. **跳转前定向验证**：在变更驱动复查通过后（或 SHA 匹配时），对照检查点的「下一步」，执行一次文件系统确认：
...
| Step 0.7 | `.docs/prd/prd.md` 存在 | 输出"⚠️ 检查点过期，重新扫描"，全量检查 |
...
| Step 5.7 / Step 6 / Step 7 | 不验证（直接跳转） | — |
| done | 不验证（直接跳转）。done 表示所有 Gate 已通过，代码已审查，无需再验证。 | — |

5. **验证通过** → 直接跳转到对应步骤，**跳过前面所有步骤的文件检查**
6. **检查点指向 `done`** → 汇报当前状态（参考「状态汇报格式」），不执行任何检查
...
接到"继续推进项目"指令后，按以下优先级依次检查：
  （步骤 0 → 步骤 0.5 → 步骤 0.6 → 步骤 0.7 → 步骤 1 → ...）
...
```
检查 .docs/prd/prd.md 是否存在：
  → 不存在：
      输出"项目尚无 PRD 产品需求文档。"
      询问用户："是否现在生成 PRD？" → 是：调用 Skill("prd-generator")，结束本轮
      → 否：输出"请准备好需求文档后重新执行 ai-master。"，结束本轮
  → 存在：继续

检查 .docs/adr/server.md 或 .docs/adr/client.md 是否存在：
  → 均不存在：
      输出"项目尚无 ADR 架构决策记录。"
      询问用户："是否现在进行技术方案设计？" → 是：调用 Skill("adr-architect")，结束本轮
      → 否：输出"请完成架构设计后重新执行 ai-master。"，结束本轮
  → 存在：进入步骤 1
```
...
| Step 0.7（PRD 不存在，调用 prd-generator） | `Step 0.7` | 下次继续检查 PRD 和 ADR |
| Step 0.7（ADR 不存在，调用 adr-architect） | `Step 0.7` | 下次继续检查 ADR |
| Step 0.7（PRD/ADR 均已存在，未调用子技能） | `Step 1` | 进入开发计划检查 |
...
> 提交 SHA: <当前 git rev-parse HEAD 的完整 SHA>
> 最后更新: <当前 ISO 8601 时间>
```
...
- 「提交 SHA」通过 `git rev-parse HEAD` 获取当前 HEAD 的完整 commit SHA；如不在 git 仓库中或命令失败，填 `—`
