      ...
      <template v-else>
        ...
        <view v-if="todayWord" class="home-page__today">
          <view class="home-page__today-label">
            <text class="iconfont home-page__today-icon">&#xe00c;</text>
            <text class="home-page__today-label-text">今日一词</text>
          </view>
          <view class="home-page__today-card" @click="goWordDetail(todayWord.id)">
            <text class="home-page__today-card-label">核心物理意象</text>
            <text class="home-page__today-card-word">{{ todayWord.word }}</text>
            <text class="home-page__today-card-phonetic">{{ todayWord.phonetic }}</text>
            <text class="home-page__today-card-meaning">{{ todayWord.coreMeaning }}</text>
            <view class="home-page__today-card-link">
              <text>查看完整解析</text>
              <text class="iconfont home-page__today-card-arrow">&#xe002;</text>
            </view>
          </view>
        </view>
        ...
        <view class="home-page__all-words">
          <view class="home-page__all-words-head">
            ...
            <text class="home-page__all-words-head-count">{{ allWords.length }} 个</text>
          </view>
...
.home-page {
  ...
  &__header {
    ...
    /* #ifdef H5 */
    backdrop-filter: blur(32rpx);  /* Phase1(src): HomeView.tsx:31 — 16px → 32rpx */
  ...
  &__today {
    ...
    &-card {
      width: 100%;
      background: linear-gradient(135deg, #1D4ED8 0%, #2563EB 50%, #3B82F6 100%); /* Phase1(src): HomeView.tsx:143 */
    ...
    &-card-arrow {
      font-size: 28rpx;      /* ArrowRight size 14 → 28rpx */
    }
  }
