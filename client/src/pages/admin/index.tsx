...
const SECTION_LABEL = {
  fontSize: '11px' as const, fontWeight: '600' as const, color: '#9CA3AF',
  letterSpacing: '2px', textTransform: 'uppercase' as const, margin: '0 0 12px',
};
const INPUT = {
...
function Overview({ libraries, words, onNavigate, onExit }: {
  ...
  const sections = [
    { id: 'libraries' as AdminSection, label: '词库管理', icon: 'book' as const, count: `${libraries.length} 个词库`, color: '#2563EB', bg: '#EFF6FF' },
    { id: 'words' as AdminSection, label: '单词管理', icon: 'font' as const, count: `${words.length} 个单词`, color: '#16A34A', bg: '#F0FDF4' },
    { id: 'users' as AdminSection, label: '用户管理', icon: 'friends' as const, count: `${mockUsers.length} 位用户`, color: '#7C3AED', bg: '#FAF5FF' },
  ];
    ...
    <View style={{ minHeight: '100vh', background: BG }}>
      ...
      <View style={{ padding: '16px 24px 40px' }}>
        ...
        <View style={{
          ...
          <Text style={{ fontSize: '11px', opacity: '0.7', letterSpacing: '2px', textTransform: 'uppercase', color: '#fff', display: 'block', marginBottom: '16px' }}>
            当前数据
          </Text>
          <View style={{ display: 'grid', gridTemplateColumns: 'repeat(3,1fr)', gap: '4px' }}>
            {[
              ...
              <View key={i} style={{ textAlign: 'center' }}>
                <Text style={{ fontSize: '30px', fontWeight: '800', color: '#fff', display: 'block', marginBottom: '2px', letterSpacing: '-0.5px' }}>{s.value}</Text>
          ...
          {sections.map(s => (
            ...
            >
              <View style={{ display: 'flex', alignItems: 'center', gap: '14px' }}>
                <View style={{ width: '44px', height: '44px', borderRadius: '12px', background: s.bg, display: 'flex', alignItems: 'center', justifyContent: 'center', flexShrink: 0 }}>
      ...
      <View style={{ minHeight: '100vh', background: BG }}>
        ...
        <View style={{ padding: '8px 24px 40px', display: 'flex', flexDirection: 'column', gap: '14px' }}>
          <View>
            ...
            <Input value={name} onInput={e => setName(e.detail.value)} placeholder='输入词库名称' style={INPUT} />
          </View>
          <View>
          ...
          <View
            ...
            style={{
              ...
              color: '#2563EB', fontSize: '15px', fontWeight: '700', textAlign: 'center',
            }}
          >
    ...
    <View style={{ minHeight: '100vh', background: BG }}>
      <PageHeader
        ...
        right={
          <View
            onClick={openNew}
            style={{ display: 'flex', alignItems: 'center', gap: '5px', padding: '8px 16px', background: '#2563EB', borderRadius: '20px' }}
          >
            ...
            {libraries.map(lib => (
              <View key={lib.id} style={{ ...CARD, padding: '20px' }}>
                ...
                <View style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between' }}>
                  <Text style={{ fontSize: '12px', color: '#2563EB', background: '#EFF6FF', padding: '3px 10px', borderRadius: '20px' }}>
                    {words.filter(w => w.libraryId === lib.id).length} 个单词
                  </Text>
                  <View style={{ display: 'flex', gap: '8px' }}>
                    <View onClick={() => openEdit(lib)} style={{ padding: '7px 16px', background: '#F1F5F9', borderRadius: '20px' }}>
                      <Text style={{ color: '#374151', fontSize: '13px', fontWeight: '500' }}>编辑</Text>
                    </View>
                    <View onClick={() => handleDelete(lib.id)} style={{ padding: '7px 16px', background: '#FEF2F2', borderRadius: '20px' }}>
    ...
    <View style={{ paddingBottom: '40px' }}>
      ...
      <View style={{ marginBottom: '20px' }}>
        ...
        <View style={{
          ...
          <Input
            value={form.word || ''}
            onInput={e => { set('word', e.detail.value); setAiDone(false); }}
      ...
      <View style={{ ...CARD, padding: '20px', marginBottom: '20px', display: 'flex', flexDirection: 'column', gap: '14px' }}>
        ...
        <View>
          ...
          <Input value={form.phonetic || ''} onInput={e => set('phonetic', e.detail.value)} placeholder='/fləʊ/' style={INPUT} />
        </View>
      </View>
      ...
      <View style={{ ...CARD, padding: '20px', marginBottom: '20px', display: 'flex', flexDirection: 'column', gap: '14px' }}>
        <View>
          ...
          <Textarea value={form.coreMeaning || ''} onInput={e => set('coreMeaning', e.detail.value)} placeholder='从物理感知出发，描述词的核心物理意义...' style={ta} />
        </View>
        <View>
          <Text style={LABEL}>核心义例句</Text>
          <Textarea value={form.coreExampleSentence || ''} onInput={e => set('coreExampleSentence', e.detail.value)} placeholder='English example sentence...' style={taShort} />
        </View>
        <View>
          <Text style={LABEL}>例句中文翻译</Text>
          <Textarea value={form.coreExampleTranslation || ''} onInput={e => set('coreExampleTranslation', e.detail.value)} placeholder='中文翻译...' style={taShort} />
        </View>
      </View>
      ...
      <View style={{ ...CARD, padding: '20px', marginBottom: '20px' }}>
        ...
        <View
          ...
          style={{
            ...
            padding: '11px', background: '#F1F5F9',
            borderRadius: '14px', color: '#374151', fontSize: '13px', fontWeight: '500',
          }}
        >
      ...
      <View style={{ display: 'flex', flexDirection: 'column', gap: '12px', marginBottom: '20px' }}>
        {(form.extendedMeanings || []).length === 0 && isNew && (
          <View style={{ textAlign: 'center', padding: '28px 0', color: '#D1D5DB', background: '#fff', borderRadius: '16px', border: '1.5px dashed #E5E7EB' }}>
        ...
        {(form.extendedMeanings || []).map((ext, i) => (
          <View key={ext.id} style={{ ...CARD, padding: '20px' }}>
            <View style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '14px' }}>
              <Text style={{ fontSize: '12px', fontWeight: '600', color: '#9CA3AF', letterSpacing: '1px', textTransform: 'uppercase' }}>
                引申义 {i + 1}
              </Text>
              {!isNew && (
                <View onClick={() => removeExt(i)} style={{ background: '#FEF2F2', borderRadius: '8px', padding: '5px 7px' }}>
            ...
            <View style={{ display: 'flex', flexDirection: 'column', gap: '12px' }}>
              <View>
                ...
                <Input value={ext.logicalEvolution} onInput={e => updateExt(i, 'logicalEvolution', e.detail.value)} placeholder='物理感知 → 抽象延伸...' style={INPUT} />
              </View>
              <View style={{ display: 'grid', gridTemplateColumns: '1fr 96px', gap: '10px' }}>
                <View>
                  <Text style={LABEL}>引申义</Text>
                  <Input value={ext.meaning} onInput={e => updateExt(i, 'meaning', e.detail.value)} placeholder='中文引申义' style={INPUT} />
                </View>
                <View>
              ...
              <View>
                ...
                <Textarea value={ext.exampleSentence} onInput={e => updateExt(i, 'exampleSentence', e.detail.value)} placeholder='English example...' style={taShort} />
              </View>
              <View>
                <Text style={LABEL}>例句中文翻译</Text>
                <Textarea value={ext.exampleTranslation} onInput={e => updateExt(i, 'exampleTranslation', e.detail.value)} placeholder='中文翻译...' style={taShort} />
              </View>
            </View>
          </View>
        ))}
      ...
      <View style={{ ...CARD, padding: '20px', marginBottom: '24px' }}>
        ...
        <Textarea value={colInput} onInput={e => setColInput(e.detail.value)} placeholder='flow freely、cash flow、go with the flow...' style={{ ...ta, minHeight: '60px' }} />
      </View>
                ...
                <View key={word.id} style={{ ...CARD, padding: '16px 20px' }}>
                  <View style={{ display: 'flex', alignItems: 'flex-start', gap: '10px' }}>
                    <View style={{ flex: 1, minWidth: '0' }}>
                      ...
                      <Text style={{ fontSize: '12px', color: '#6B7280', display: 'block', marginBottom: '8px', lineHeight: '1.5', overflow: 'hidden', textOverflow: 'ellipsis', whiteSpace: 'nowrap' }}>
                        {word.coreMeaning}
                      </Text>
                      <View style={{ display: 'flex', gap: '6px', flexWrap: 'wrap' }}>
                        {lib && <Text style={{ fontSize: '11px', color: '#2563EB', background: '#EFF6FF', padding: '2px 8px', borderRadius: '20px' }}>{lib.name}</Text>}
                        <Text style={{ fontSize: '11px', color: '#6B7280', background: '#F3F4F6', padding: '2px 8px', borderRadius: '20px' }}>{word.extendedMeanings.length} 个引申义</Text>
                      </View>
                    </View>
                    <View style={{ display: 'flex', gap: '8px', flexShrink: 0 }}>
                      <View onClick={() => setEditWord(word)} style={{ padding: '7px 14px', background: '#F1F5F9', borderRadius: '20px' }}>
                        <Text style={{ color: '#374151', fontSize: '13px', fontWeight: '500' }}>编辑</Text>
                      </View>
                      <View onClick={() => handleDelete(word.id)} style={{ padding: '7px 14px', background: '#FEF2F2', borderRadius: '20px' }}>
                        <Text style={{ color: '#DC2626', fontSize: '13px', fontWeight: '500' }}>删除</Text>
                      </View>
                    </View>
                  </View>
          ...
          {mockUsers.map(u => (
            <View key={u.id} style={{ ...CARD, padding: '16px 20px', display: 'flex', alignItems: 'center', gap: '14px' }}>
              ...
              <View style={{ textAlign: 'right', flexShrink: 0 }}>
                <Text style={{ fontSize: '17px', fontWeight: '700', color: '#2563EB', display: 'block', marginBottom: '2px' }}>{u.learnedWords}</Text>
