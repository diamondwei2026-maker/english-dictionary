...
function LightImage() {
  return (
    <View style={{ width: '100%', height: '100%' }}>
      <svg viewBox='0 0 400 220' fill='none' xmlns='http://www.w3.org/2000/svg' style={{ width: '100%', height: '100%' }}>
        <rect width='400' height='220' rx='20' fill='#FEFCE8' />
        {/* 小程序不支持 <defs>/<radialGradient>，用多层同心圆叠加模拟发光 */}
        {[48, 38, 28, 18, 8].map((r, i) => (
          <circle key={i} cx='200' cy='110' r={r} fill='#FDE047' opacity={0.06 + 0.06 * (5 - i)} />
        ))}
        {/* 主光线 — 8 方向 */}
...
export function PhysicalImage({ type }: PhysicalImageProps) {
  const image = imageMap[type] || <GenericImage />;
  return (
    <View style={{ width: '100%', borderRadius: '16px', overflow: 'hidden' }}>
      {image}
    </View>
  );
}
